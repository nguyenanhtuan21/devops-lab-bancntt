...
export default {
  setup() {
    ...
    const numberOfCompletedTodos = computed(
      () => todos.value.filter((todo) => todo.isFinished).length
    );
    ...
    const onFinished = (e, ele) => {
      console.log(e.target.value, ele.isFinished);
      let value = !ele.isFinished;
      let fakeTodo = { ...ele, isFinished: !ele.isFinished };
      axiosInstance
        .put("/api/tasks", fakeTodo)
        .then((res) => {
          ele.isFinished = value;
        })
        .catch((e) => {
          console.log(e);
        });
